...
function ShowClickIconPhone() {
    ...
    brands.forEach(function (item) {
        html +=
            `<div class="col shadow-lg p-3 bg-body rounded itemIconShow mt-2" style="cursor: pointer" onclick="RemoveIconShow('${item}')">
...
btnview.forEach(function (item) {
    item.addEventListener("click", function () {
        let name = item.getAttribute("value").toLowerCase();
...
let urlcard = document.querySelectorAll(".btnBuyCard");
function buyProduct(){
    $.ajax({
        type: "GET",
        url: "/san-pham/dien-thoai/show",
        success: function (data) {
            alert("thanh cong")
        },
        error: function (data) {
