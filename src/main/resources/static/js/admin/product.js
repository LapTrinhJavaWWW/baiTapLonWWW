...
let indexPage = 0;
// event pagination product item nao click add class active con lai remove active
...
pageitem.forEach(function (item) {
	item.addEventListener('click', function (e) {
		...
		indexPage = this.value;
		$.ajax({
			url: '/admin/product?page=' + this.value,
			type: 'POST',
			data: this.value,
			success: function (data) {
				GenerateTable(data);
...
function GenerateTable(data) {
	...
	data.forEach(function (item) {
		...
		<img src="${item.img[0]}" alt="" style="max-width: 50%" />
		</td>
		<td class="text-center">${item.screen}</td>
		<td>${item.chip}</td>
		<td class="text-center">${item.ram}</td>
		<td class="text-center">${item.rom}</td>
		<td class="text-center">10</td>
		<td>${item.bestSeller}</td>
		<td style="cursor: pointer"><i class="fas fa-pen openModelUpdate" value="${item}" data-bs-toggle="modal" data-bs-target="#exampleModalUpdate"></i></td>
		<td style="cursor: pointer">
			<i class="fas fa-trash openModelDel" value="${item.id}" data-bs-toggle="modal" data-bs-target="#exampleModal"></i>
		</td>
	</tr>`;
		i++;
	});
	$('#tableBodyDs').html(html);
	$('#tableds').css('height', '10vh');
}
